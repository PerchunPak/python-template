...
def skip_data_from_docs(
    ...
    """Skip ``log`` function {% if cookiecutter.other_languages_support == 'y' -%}and ``_`` attribute everywhere. And skip all if language is not English.{% else %}everywhere.{% endif %}"""
    {% if cookiecutter.other_languages_support == 'y' -%}
if language != "en":
        return True

    {% endif %}if what == "data" and name.endswith(".log"):
        skip = True
    {% if cookiecutter.other_languages_support == 'y' -%}
